    ...
    return (
        <>
            ...
            <div className="mb-6 p-4 bg-[var(--accent)]/10 border border-[var(--accent)]/30 rounded-lg">
                <h3 className="font-semibold text-[var(--text-primary)] mb-2">{t("partners_tab.approved.demo_title")}</h3>
                <div className="flex items-center gap-4">
                    <div className="font-mono text-lg bg-[var(--bg-card)] px-3 py-2 rounded border border-[var(--border-color)] text-[var(--text-primary)]">
                        PartnerPortal@123
                    </div>
                            ...
                            {(
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="font-semibold text-sm text-[var(--text-primary)]">{t("partners_tab.common.name")}:</label>
                                            <p className="text-sm text-[var(--text-primary)]">{selectedPartner.name}</p>
                                        </div>
                                        <div>
                                            <label className="font-semibold text-sm text-[var(--text-primary)]">{t("partners_tab.common.email")}:</label>
                                            <p className="text-sm text-[var(--text-primary)]">{selectedPartner.email}</p>
                                        </div>
                                        <div>
                                            <label className="font-semibold text-sm text-[var(--text-primary)]">{t("partners_tab.common.number")}:</label>
                                            <p className="text-sm text-[var(--text-primary)]">{selectedPartner.number}</p>
                                        </div>
                                        <div>
                                            <label className="font-semibold text-sm text-[var(--text-primary)]">{t("partners_tab.approved.registered")}:</label>
                                            <p className="text-sm text-[var(--text-primary)]">{formatDate(selectedPartner.registeredAt)}</p>
                                        </div>
                                    </div>
                                    ...
                                    {selectedPartner.message && (
                                        <div>
                                            <label className="font-semibold text-sm text-[var(--text-primary)]">{t("partners_tab.approved.message")}:</label>
                                            <p className="text-sm bg-[var(--accent)]/10 text-[var(--text-primary)] p-3 rounded mt-1">
                                                {selectedPartner.message}
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    {/* Coupons */}
                                    <div>
                                        <h4 className="font-semibold mb-2 text-[var(--text-primary)]">{t("partners_tab.approved.assigned_coupons")}:</h4>
                                        {selectedPartner.coupons?.length === 0 ? (
                                            <p className="text-[var(--text-secondary)] text-sm">{t("partners_tab.approved.no_coupons")}</p>
                                        ) : (
                                            <div className="grid grid-cols-1 gap-2">
                                                {selectedPartner.coupons?.map(coupon => (
                                                    <div key={coupon.id} className="border border-[var(--border-color)] rounded px-3 py-2 flex items-center justify-between">
                                                        <div>
                                                            <div className="font-mono text-sm bg-[var(--accent)]/10 text-[var(--text-primary)] px-2 py-1 rounded mb-1 inline-block">
                                                                {coupon.code}
                                                            </div>
