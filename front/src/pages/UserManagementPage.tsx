...
interface User {
  ...
  createdAt: string;
  updatedAt: string;
  signupSource?: string; // google | facebook | twitter | credentials
}
      ...
      <DialogContent className="bg-[var(--bg-card)] border-[var(--border-color)] max-w-md">
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-3">
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.name")}</p>
              <p className="font-medium text-[var(--text-primary)]">
                {user.name || t("user_management.fields.not_provided")}
              </p>
            </div>
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.email")}</p>
              <p className="font-medium text-[var(--text-primary)]">{user.email}</p>
            </div>
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.phone")}</p>
              <p className="font-medium text-[var(--text-primary)]">
                {user.phone || t("user_management.fields.not_provided")}
              </p>
            </div>
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.email_verified")}</p>
              {user.emailVerified ? (
            ...
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.role")}</p>
              <Badge className="bg-[var(--bg-secondary)] text-[var(--text-secondary)] border-[var(--border-color)] text-xs font-medium">
                {user.role}
              </Badge>
            </div>
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.signup_source")}</p>
              <Badge className="bg-[var(--accent)]/10 text-[var(--accent)] border-[var(--accent)]/30 text-xs font-medium">
                {t(`user_management.signup_source.${user.signupSource ?? "credentials"}`)}
              </Badge>
            </div>
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.created_at")}</p>
              <p className="font-medium text-[var(--text-primary)]">
                {formatDate(user.createdAt)}
              </p>
            </div>
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">{t("user_management.fields.last_updated")}</p>
              <p className="font-medium text-[var(--text-primary)]">
        ...
        <CardContent className="p-0">
          {loading && !users.length ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-base text-[var(--text-secondary)]">{t("user_management.loading")}</p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-lg font-semibold text-[var(--text-primary)] mb-1.5">{t("user_management.error_title")}</p>
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-1.5">{t("user_management.no_users")}</h3>
              {searchQuery && (
                <p className="text-sm text-center text-[var(--text-secondary)] mb-6 max-w-sm">
          ...
          {!loading && !error && users.length > 0 && (
            <div className="flex items-center justify-between border-t border-[var(--border-color)] px-6 py-4">
              <div className="text-sm text-[var(--text-secondary)]">
                {t("user_management.showing")} <span className="font-medium text-[var(--text-primary)]">{users.length}</span> {t("user_management.of")}{" "}
                <span className="font-medium text-[var(--text-primary)]">{totalUsers}</span> {t("user_management.users_count")}
              </div>
              <Pagination>
                <PaginationContent>
                  <PaginationItem>
