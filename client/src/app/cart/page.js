        ...
        <div className="md:col-span-6 flex items-center">
          ...
          <div className="flex-1 min-w-0">
            <Link
              ...
              className="font-semibold text-gray-900 hover:text-brand-brown transition-colors line-clamp-2"
            >
              {productName}
            </Link>
            {variantName && (
        ...
        <div className="md:col-span-2 flex items-center justify-between md:justify-center">
          ...
          <div className="flex items-center border-2 border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <button
              ...
              className="px-3 py-2 hover:bg-brand-brown hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="w-20 h-1 bg-brand-brown"></div>
        </div>
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="bg-white p-8 md:p-12 rounded-lg shadow-md text-center border border-gray-200 max-w-2xl mx-auto">
          ...
          <Link href="/products">
            <Button className="bg-brand-brown hover:bg-brand-dark text-white px-8 py-6 text-base font-semibold shadow-lg hover:shadow-xl transition-all duration-200">
              Continue Shopping
            </Button>
          </Link>
        </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {!isAuthenticated && cart.items.length > 0 && (
        <div className="bg-gradient-to-r from-brand-brown/5 via-brand-dark/5 to-brand-brown/5 border-l-4 border-brand-brown p-6 rounded-lg flex items-start mb-6 shadow-md hover:shadow-lg transition-shadow duration-300">
          <div className="flex-shrink-0 mr-4">
            <div className="w-14 h-14 bg-brand-brown rounded-full flex items-center justify-center shadow-lg">
              <svg
          ...
          <div className="flex-1">
            <h2 className="text-xl font-bold text-gray-900 mb-2 flex items-center gap-2">
              Guest Shopping Cart
              <span className="text-xs font-normal bg-brand-brown/10 text-brand-brown px-2 py-1 rounded-full">
                Login Required
              </span>
            </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link href="/auth?redirect=cart">
                <Button className="bg-brand-brown hover:bg-brand-dark text-white font-semibold px-6 py-3 rounded-lg shadow-md hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02]">
                  Log In to Continue
                </Button>
              </Link>
              <Link href="/auth?redirect=cart">
                <Button
                  variant="outline"
                  className="border-2 border-brand-brown hover:bg-brand-brown hover:text-white text-brand-brown font-semibold px-6 py-3 rounded-lg transition-all duration-200"
                >
                  Create Account
                </Button>
              </Link>
      ...
      {/* Show merge progress */}
      {mergeProgress && (
        <div className="bg-brand-brown/10 border-l-4 border-brand-brown p-4 rounded-md flex items-start mb-6 shadow-sm">
          <Loader2 className="text-brand-brown mr-3 mt-0.5 flex-shrink-0 animate-spin" />
          <div>
            <h2 className="text-lg font-semibold text-brand-brown">
              Merging Cart
            </h2>
            <p className="text-brand-dark">{mergeProgress}</p>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-300">
            ...
            <div className="p-4 md:p-5 border-t bg-gradient-to-r from-gray-50 to-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3">
              <Link href="/products">
                <Button
                  variant="outline"
                  className="border-brand-brown text-brand-brown hover:bg-brand-brown hover:text-white transition-all duration-200"
                >
                  ← Continue Shopping
                </Button>
              </Link>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-md border border-gray-200 p-5 md:p-6 sticky top-4 hover:shadow-lg transition-shadow duration-300">
            <div className="mb-6">
              ...
              <div className="w-16 h-1 bg-brand-brown"></div>
            </div>

            {/* Apply Coupon - Available for both guests and logged-in users */}
            {cart.items?.length > 0 && (
              <div className="mb-6 p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200">
                ...
                {coupon ? (
                  <div className="flex justify-between items-start bg-gradient-to-r from-brand-cream to-amber-50 p-4 rounded-lg border-2 border-brand-gold">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="font-bold text-brand-brown text-base">
                          {coupon.code}
                        </span>
                        <span className="text-xs bg-brand-gold/40 text-brand-brown px-2 py-1 rounded-full font-semibold">
                          Applied
                        </span>
                      </div>
                    ...
                    <form onSubmit={handleApplyCoupon} className="flex gap-2">
                      <Input
                        ...
                        className={`flex-1 border-2 ${couponError
                          ...
                          : "border-gray-300 focus-visible:ring-brand-brown"
                          }`}
                      />
                      <Button
                        type="submit"
                        disabled={couponLoading}
                        className="bg-brand-brown hover:bg-brand-dark text-white border-0"
                      >
                        {couponLoading ? (
            ...
            <div className="border-t-2 border-gray-200 pt-5">
              <div className="space-y-3 text-sm">
                ...
                {coupon && (
                  <div className="flex justify-between items-center bg-brand-cream px-3 py-2 rounded">
                    <span className="text-brand-brown font-medium">Discount</span>
                ...
                <div className="flex justify-between items-center py-1">
                  ...
                  {totals.shipping > 0 ? (
                    ...
                    <span className="text-brand-brown font-bold text-base flex items-center gap-1">
                      <svg
                        className="w-4 h-4"
                        fill="none"
                        stroke="currentColor"
            ...
            {/* Checkout Button */}
            <Button
              className="w-full mt-6 bg-brand-brown hover:bg-brand-dark text-white shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02] font-semibold py-6 text-base"
              size="lg"
