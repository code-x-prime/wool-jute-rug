...
/**
 * @desc    Get all custom rug requests with pagination and filtering
 * @route   GET /api/admin/content/custom-rugs
 * @access  Private/Admin
 */
const getCustomRugRequests = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Get total count for pagination
    const totalSubmissions = await prisma.customRugRequest.count({ where });
    const totalPages = Math.ceil(totalSubmissions / limitNumber);

    const submissions = await prisma.customRugRequest.findMany({
      where,
      orderBy: { createdAt: "desc" },
      skip,
      take: limitNumber,
...
/**
 * @desc    Get a single custom rug request by ID
 * @route   GET /api/admin/content/custom-rugs/:id
 * @access  Private/Admin
 */
const getCustomRugRequestById = asyncHandler(async (req, res) => {
  const { id } = req.params;

  try {
    const submission = await prisma.customRugRequest.findUnique({
      where: { id },
    });

    if (!submission) {
      throw new ApiError(404, "Custom rug request not found");
    }

    return res.status(200).json(new ApiResponsive(200, submission));
  } catch (error) {
    console.error("Error fetching custom rug request:", error);
    if (error instanceof ApiError) {
      throw error;
    }
    throw new ApiError(500, "Failed to fetch custom rug request");
  }
});

/**
 * @desc    Update a custom rug request's status and notes
 * @route   PUT /api/admin/content/custom-rugs/:id
 * @access  Private/Admin
 */
const updateCustomRugRequest = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    const updatedSubmission = await prisma.customRugRequest.update({
      where: { id },
      data: {
        ...(status && { status }),
    ...
    throw new ApiError(500, "Failed to update custom rug request");
  }
});

/**
 * @desc    Delete a custom rug request
 * @route   DELETE /api/admin/content/custom-rugs/:id
 * @access  Private/Admin
 */
const deleteCustomRugRequest = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    await prisma.customRugRequest.delete({
      where: { id },
    });
    ...
    return res
      ...
      .json(
        new ApiResponsive(200, {
          message: "Custom rug request deleted successfully",
        })
      );
  } catch (error) {
    ...
    throw new ApiError(500, "Failed to delete custom rug request");
  }
});
