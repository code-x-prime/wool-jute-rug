...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Your OTP for Email Verification - wooljuterug",
...
export const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email,
      subject: "Reset Your Password - wooljuterug",
...
export const requestAccountDeletion = asyncHandler(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Confirm Account Deletion - wooljuterug",
...
export const addReview = asyncHandler(async (req, res, next) => {
  ...
  if (!productId || isNaN(rating)) {
    throw new ApiError(400, "Product ID and valid rating are required");
  }
  ...
  if (req.files && req.files.length > 0) {
    ...
    const uploadPromises = filesToProcess.map(file => processAndUploadImage(file, "reviews"));
