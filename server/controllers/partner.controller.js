...
export const getPartnerDashboard = asyncHandler(async (req, res) => {
    ...
    // Commission is on order value AFTER discount (subTotal - discount), not on total (which includes shipping)
    ...
    orders.forEach(order => {
        ...
        const commissionEarned = (commissionBase * commissionPercentage) / 100;

        totalOrderValue += commissionBase;
...
export const getPartnerCoupons = asyncHandler(async (req, res) => {
    ...
    const couponsWithEarnings = await Promise.all(
        coupons.map(async (couponPartner) => {
            ...
            // Commission is on order value AFTER discount (subTotal - discount)
            ...
            ordersForCoupon.forEach(order => {
                ...
                actualEarnings += (commissionBase * commissionRate) / 100;
            });
...
export const getPartnerEarnings = asyncHandler(async (req, res) => {
    ...
    const earningsData = orders.map(order => {
        ...
        const commissionBase = parseFloat(order.subTotal || 0) - parseFloat(order.discount || 0);
        const earningAmount = (commissionBase * commissionRate) / 100;
