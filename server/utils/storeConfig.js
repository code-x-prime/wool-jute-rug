...
export const getStoreConfig = () => {
  return {
    ...
    // Email Configuration
    fromName: process.env.FROM_NAME || process.env.STORE_NAME || "Your Store",
    fromEmail:
...
export async function getStoreConfigFromDb() {
  try {
    ...
    if (siteSettings) {
      return {
        storeName: siteSettings.siteName || process.env.STORE_NAME || "Your Store",
        storeEmail: siteSettings.siteEmail || process.env.STORE_EMAIL || "store@example.com",
        storePhone: siteSettings.sitePhone || process.env.STORE_PHONE || "",
        storeAddress: siteSettings.siteAddress || process.env.STORE_ADDRESS || "",
        storeTagline: siteSettings.siteDescription || process.env.STORE_TAGLINE || "Premium Quality Products",
        storeDescription: siteSettings.siteDescription || process.env.STORE_DESCRIPTION || "",
        fromName: siteSettings.siteName || process.env.FROM_NAME || "Your Store",
        fromEmail: siteSettings.siteEmail || process.env.FROM_EMAIL || "store@example.com",
        websiteUrl: process.env.WEBSITE_URL || "https://yourstore.com",
        supportEmail: siteSettings.siteEmail || process.env.SUPPORT_EMAIL || "support@example.com",
